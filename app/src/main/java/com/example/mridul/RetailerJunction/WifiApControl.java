...
public class WifiApControl {
    ...
    private WifiApControl(WifiManager mgr, Context context){
        ...
        servalConfiguration.SSID = "RetailFreeWiFi";
        servalConfiguration.allowedKeyManagement.set(KeyMgmt.NONE);

        WifiConfiguration config = readProfile("saved_user_ap", context);
    ...
    public int getWifiApState(){
        try {
            ...
            Log.v("org.WifiConnect",e.toString(),e); // shouldn't happen
            return -1;
        }
    }
    ...
    public boolean isUserConfig(Context context){
        WifiConfiguration config = getWifiApConfiguration();
        boolean ret = isUserConfig(config);
        if (ret){
            saveProfile("saved_user_ap", config, context);
    ...
    public boolean enable(boolean withUserConfig, Context context){
        WifiConfiguration config = getWifiApConfiguration();
    ...
    private WifiConfiguration readProfile(String name, Context context) {
    ...
    private void saveProfile(String name, WifiConfiguration config, Context context) {
